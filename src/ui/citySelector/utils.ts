export const getCityMap = (citiesSorted: string[]): Map<string, string[]> => {
    ...
    if (citiesSorted.length >= 1 && !cityMap.get(citiesSorted[citiesSorted.length - 1][0]))
        cityMap.set(citiesSorted[citiesSorted.length - 1][0], [citiesSorted[citiesSorted.length - 1]]);
    if (citiesSorted.length >= 2 && citiesSorted[citiesSorted.length - 1][0] === citiesSorted[citiesSorted.length - 2][0]) {
